 ...
 */
AHolybroX500::AHolybroX500() {
    // Set this pawn to call Tick() every frame.
    PrimaryActorTick.bCanEverTick = true;

    // Set the defualt controller
    AIControllerClass = LoadClass<AController>(NULL, TEXT("/Script/Holodeck.HolybroX500Controller"), NULL, LOAD_None, NULL);
 ...
 */
void AHolybroX500::InitializeAgent() {
    ...
    // UE_LOG(LogHolodeck, Fatal, TEXT("Agent inicializado!"));
}
 ...
 */
void AHolybroX500::Tick(float DeltaTime) {
    ...
    // UE_LOG(LogHolodeck, Fatal, TEXT("Dentro do tick"));
}

/**
 * @brief Toggles whether aerodynamic forces are calculated in the main loop.
 */
void AHolybroX500::ForcesInMainController(bool SetIt){
    bForcesInMain = SetIt;
}
 ...
 */
void AHolybroX500::ApplyFlightForce(){
    FVector Velocity = RootMesh->GetPhysicsLinearVelocity();
    float Speed = Velocity.Size();  
    FVector DragDirection = Velocity.GetSafeNormal();

    FVector DragForce = -0.5f * _M_AIR_DENSITY * _M_DRAG_COEFF * 0.4f * Speed * DragDirection;
 ...
 */
void AHolybroX500::ApplyThrusters(float* const ThrusterArray){
